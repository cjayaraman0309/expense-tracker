...
- `POST /expenses/<int:id>/delete` — verifies ownership, deletes the expense row, redirects to `/profile` — logged-in only
...
- Modify `templates/profile.html`:
  - Inside the existing "Actions" `<td>` per transaction row, add a delete form:
    - `<form>` with `method="POST"` and `action` set to `/expenses/{{ tx.id }}/delete`
...
- [ ] POSTing to `/expenses/<id>/delete` for a non-existent or another user's expense returns 404
