...
- `POST /expenses/add` — validates and inserts the new expense — logged-in only
...
- Create `templates/add_expense.html`:
  - Extends `base.html`
  - Form with `method="POST"` and `action` set to `url_for("add_expense")`
  - Fields:
...
|---|---|---|
| `insert_expense` | valid `user_id`, `amount=50.0`, `category="Food"`, `date="2026-03-20"`, `description="Lunch"` | Row inserted; querying the DB returns the new row with correct values |
| `insert_expense` | valid `user_id`, `description=None` | Row inserted with `description` stored as `NULL` |
