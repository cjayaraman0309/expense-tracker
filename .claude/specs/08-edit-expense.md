...
- `GET /expenses/<int:id>/edit` — renders the edit form pre-populated with existing expense values — logged-in only
- `POST /expenses/<int:id>/edit` — validates and saves the updated expense — logged-in only
...
- Create `templates/edit_expense.html`:
  - Extends `base.html`
  - Form with `method="POST"` and `action` set to `/expenses/{{ expense.id }}/edit`
...
- `app.py`
  ...
  - Replace the `GET`-only placeholder at `/expenses/<int:id>/edit` with a full `GET` + `POST` handler:
    - `GET`: call `get_expense_by_id`; return 404 if not found or not owned; render `edit_expense.html` with the expense and categories list
    - `POST`: read form fields, validate (same rules as Step 07), call `update_expense`, redirect to `url_for("profile")` on success; re-render the form with errors otherwise
...
|---|---|---|
| `get_expense_by_id` | valid `expense_id` and correct `user_id` | Returns the matching row as a dict-like object with correct field values |
