...
- `templates/profile.html` — confirm the ₹ symbol is used for all currency display
...
- `database/queries.py` — pure query helpers with no Flask imports; one function per data concern:
  ...
  - `get_recent_transactions(user_id, limit=10)` → list of dicts, each with `date`, `description`, `category`, `amount`; ordered newest-first
  - `get_category_breakdown(user_id)` → list of dicts, each with `name`, `amount`, `pct`; `pct` is percentage of total rounded to nearest int; ordered by `amount` descending
...
| `GET /profile` unauthenticated | no session | redirects to `/login` (302) |
| `GET /profile` authenticated as seed user | `session["user_id"]` = demo user id | 200; body contains `"Demo User"`, `"demo@spendly.com"`, `₹` symbol, `total_spent` = 346.24, `transaction_count` = 8, `top_category` = `"Bills"`, transactions in newest-first order, 7 categories in breakdown |
